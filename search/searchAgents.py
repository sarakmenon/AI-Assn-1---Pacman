...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        """
        "*** YOUR CODE HERE ***"
        # get visited corners from the goal state
    ...
    def getSuccessors(self, state: Any):
        ...
        # get current position and visited corners
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            # find the next position for this action - from code snippet above
            x, y = currentPosition
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)

            # skip moves that hit a wall - also from code snippet above
            hitsWall = self.walls[nextx][nexty]
            # if we hit wall, skip
            if hitsWall:
                continue
            ...
            # copy visited corners
            next_visited = list(visited_corners)

            # mark the corner visited if the next position is a corner
            if next_position in self.corners:
                corner_index = self.corners.index(next_position)
                next_visited[corner_index] = True

            # add the next state, action, and step cost of 1
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for order in permutations(unvisited_corners):
        ...
        for corner in order:
            # using Manhattan distance as heuristic, since it is admissible and consistent
            total_cost += abs(position[0] - corner[0]) + abs(position[1] - corner[1])
            position = corner

        # keep the lowest cost across all orders
